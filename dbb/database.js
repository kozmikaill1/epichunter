...
// users tablosu (exp ve money dahil)
...
// items tablosu (kullanıcının envanteri için)
...
// Kullanıcıyı kayıt et ya da mesaj sayısını artır
...
function addItem(userId, itemName) {
  return new Promise((resolve, reject) => {
    db.get(`SELECT quantity FROM items WHERE userId = ? AND itemName = ?`, [userId, itemName], (err, row) => {
      ...
      if (!row) {
        db.run(
          `INSERT INTO items (userId, itemName, quantity) VALUES (?, ?, 1)`,
          [userId, itemName],
          function(err) {
            if (err) reject(err);
            else resolve();
        ...
        db.run(
          `UPDATE items SET quantity = quantity + 1 WHERE userId = ? AND itemName = ?`,
          [userId, itemName],
          function(err) {
            if (err) reject(err);
            else resolve();
